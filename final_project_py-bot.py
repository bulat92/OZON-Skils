...
#запрещенные слова
banned_words = ['dick', 'vagina', 'cunt', 'cock', 'ass', 'prostitute', 'slut', 'whore', 'fuck', 'shit']

# ключевые слова по которым будут подбиратся анекдоты
word_key = {'богат': 0, 'рестора': 0, 'бедн': 0, 'работ': 0, 'прививк': 0, 'covid-19': 0, 'коронавирус': 0,
            'вирус': 0, 'мяс': 1, 'растен': 1, 'коров': 1,
            'веган': 1, 'пенальт': 2, 'финал': 2, 'правда': 2, 'ложь': 2, 'обман': 2, ' фальшь': 2, 'vip': 3, 'qr': 3, 'коды': 3, 'мероприят': 3,
...
def for_anecdotes_editing(mes):
    ...
    for ban_word in banned_words:
        if re.search(rf'{ban_word}', text):
            ...
            return # прекратить выполнение функций если есть запрещенные слова
    ...
    for word, value in word_key.items():
        if re.search(rf'{word}', text):
            anecdotes_for_send.append(array_anecdotes[value]) # если нашлись ключевые слова то добавить анекдот в массив 
    ...
    if len(anecdotes_for_send) > 1:
        ...
        for anecdot in enumerate(anecdotes_for_send):
            if anecdot[0] > len(word_number):
                return # если анекдотов нашлось больше чем длина массива word number прекратить выполнение функций

            bot.send_message(mes.chat.id, f'Вот {word_number[anecdot[0]]} из них {anecdot[1]}')

    if len(anecdotes_for_send) == 1:
        bot.send_message(mes.chat.id, f'Кажется я вспомнил несмешной анекдот {anecdotes_for_send[0]}') #если анекдот нашелся только один
